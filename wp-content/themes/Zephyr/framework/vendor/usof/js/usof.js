...
!function($){
	...
	$usof.field['group'] = {
		...
		init: function(){
			this.$btnAddGroup = this.$row.find('.usof-form-group-add');

			console.log(0);
			console.log(this.$btnAddGroup);
		}
			...
			$.each($container.find('.usof-form-row, .usof-form-wrapper, .usof-form-group'), function(index, elm){
				...
				if (isRow) {
					...
					var $btnAddGroup = $field.find('.usof-form-group-add'),
						...
						index = 0;// TODO: load actual index when complete save process
					...
					$btnDelGroup.live('click', function(){ // TODO: check whether adding event for each button is leaner
						event.stopPropagation();
						if (!confirm('Are you sure you want to delete this group?')) return false; // TODO: i18n

						$group = $(this).closest('.usof-form-wrapper');
						$group.remove();
					});
				}
			}.bind(this));
